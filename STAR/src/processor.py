...
class processor(object):
    ...
    def load_model(self):
        ...
        if self.args.load_model is not None:
            ...
            if os.path.isfile('./output/hotel/star/star_290.tar'):
                print('Loading checkpoint')
                checkpoint = torch.load('./output/hotel/star/star_290.tar')
    ...
    def test_epoch(self):
        ...
        for batch in tqdm(range(self.dataloader.testbatchnums)):
            ...
            all_output = all_output[:, 7:]
            pred_abs = all_output + shift_value[0]

            for q in range(20):
                t = batch_abs[:8, i, 1:].cpu().numpy()
                x = batch_abs[:8, i, :1].cpu().numpy()
                plt.plot(x, t, marker='o', color='blue', label='Observation')
                plt.plot(batch_abs[8:, i, :1].cpu().numpy(), batch_abs[8:, i, 1:].cpu().numpy(), marker='o', color='green', label='Ground Truth')

                plt.plot(pred_abs[q, :, i, 0].cpu().numpy(), pred_abs[q, :, i, 1].cpu().numpy(), marker='o', color='yellow', label='Predictions')
                plt.xlim(3.5, 8)
                plt.ylim(-1, 14)
